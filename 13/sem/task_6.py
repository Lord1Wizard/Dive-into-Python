...
class UserLevelExeption(UserExeption):
    ...
    def __str__(self):
        return f'Ошибка уровня доступа. Ваш уровень {self.value} меньше создаваемого уровня - {self.other}'
...
class UserPermissionExeption(UserExeption):
    ...
    def __str__(self):
        return f'Ошибка доступа. Пользователя {self.user_name} с ID {self.user_id} не существует'
...
if __name__ == '__main__':
    ...
    def login():
        ...
        for user in user_list:
            if new_user == user:
                cur_user = user
        if cur_user == None:
            raise UserPermissionExeption(name, pers_id)
        else:
            return cur_user
    ...
    while True:
        ...
        if cur_user.user_level <= level:
            ...
            raise UserLevelExeption(cur_user.user_level, level)
